...
function UserCard({
  ...
  return (
    <article className="card-gaming flex aspect-square flex-col p-4">
      <div className="flex flex-1 flex-col items-center text-center">
        ...
        <p className="mt-1 text-xs font-semibold text-purple-300/70">
          {ROLE_LABELS[user.role]}
        </p>
        <p className="mt-2 font-game text-lg font-bold text-cyan-400">
          {user.karmaScore} KP
        </p>
      </div>

      {isSuperAdmin ? (
        <p className="mt-3 text-center text-xs font-semibold text-purple-300/70">
          Rôle non modifiable
        </p>
      ) : (
        <GamingSelect
    ...
    <AuthGuard>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <p className="mb-4 text-sm text-purple-300/60">
          Assigne les rôles Modérateur, Admin ou Utilisateur.
        </p>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {loading
            ...
            : filtered.map((u) => (
                <UserCard key={u.id} user={u} onRoleChange={changeRole} />
              ))}
        </div>

        {!loading && filtered.length === 0 && (
          <p className="py-12 text-center text-purple-300/50">
            {users.length === 0
              ? "Aucun utilisateur"
